...
/*   Updated: 2019/04/04 12:01:56 by ztrouill    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void			ft_bresenham_vert(t_utilities *utilities, t_point *point,
					t_image image, t_segment segment)
{
	while (segment.i++ <= segment.sy)
	{
		...
		{
			if (point->coord[point->coord_i + 1][point->coord_j][2] == 0 &&
...
static void			ft_bresenham_vert_2(t_utilities *utilities, t_point *point,
					t_image image, t_segment segment)
{
	segment.c = 'h';
	while (segment.i++ <= segment.sx)
					...
					t_image image, t_segment segment)
{
	...
	if (segment.c == 'v')
	{
		if (segment.sx < segment.sy)
			ft_bresenham_vert(utilities, point, image, segment);
		if (segment.sy < segment.sx)
			ft_bresenham_vert_2(utilities, point, image, segment);
	}
	if (segment.c == 'h')
	{
		if (segment.sx < segment.sy)
